...
if __name__ == '__main__':
    ...
    # values = amount of energy consumed (negative) or generated (positive)
    # x und y --> Koordinaten
    assets = read_csv('assets.csv').set_index(['id', 'type', 'x', 'y'])

    # define cost_matrix; in this case cost = distance
    from scipy.spatial import distance_matrix
    assets_xy = assets.reset_index().filter(items=['id', 'x', 'y'], axis=1).set_index('id')
    cost_matrix = DataFrame(distance_matrix(assets_xy.values, assets_xy.values), index=assets_xy.index,
                            columns=assets_xy.index).astype('int64')
    ...
    assets_idx = assets.index  # store index for later
    assets = assets.reset_index().drop(columns=['x', 'y', 'type']).set_index('id')
    # endregion

    # region loop over timesteps
    for t in assets.columns[:10]:
