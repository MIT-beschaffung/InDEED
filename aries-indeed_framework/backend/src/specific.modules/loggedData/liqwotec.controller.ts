...
export class LiqwotecController {
    ...
    @Post('logData')
    @ApiOperation({summary: 'Records new logged data for Liqwotec'})
    ...
    recordLogData(@Body() completeBody) {
        ...
        const result = this.loggedDataService.createOne(completeBody, databases.LiqwotecDataDB, 'Liqwotec',);
    ...
    async getAllLogData() {
        ...
        try {
            const result = await this.databaseService.getAllIDs(databases.LiqwotecDataDB);
    ...
    async getLogDataById(@Param('id') id: string) {
        ...
        try {
            const res = await this.databaseService.findByID(id, databases.LiqwotecDataDB);
            ...
            this.MyLogger.error('The following error occurred while trying to get the document with this id: ' +
                    id + 'in this database' + databases.LiqwotecDataDB + '\n' + error);
            return error;
        }
    }
    ...
    async deleteLogDataById(@Param('id') id: string) {
        ...
        try {
            const res = await this.databaseService.deleteByID(id, databases.LiqwotecDataDB);
            ...
            this.MyLogger.error('The following error occurred while trying to delete in ' +
                    databases.LiqwotecDataDB + 'for this id: ' + id + '\n this is the error statement: ' + error);
            return error;
        }
    }
    ...
    async getLogDataByTime(@Body('upperbound') upperbound: number, @Body('lowerbound') lowerbound: number) {
        ...
        try {
            return await this.databaseService.findManyByTime(upperbound, lowerbound, databases.LiqwotecDataDB);
        } catch (error) {
            this.MyLogger.error('The following error occurred while trying to get documents within this time period: ' +
                    lowerbound + ' : ' + upperbound + 'in this database' + databases.LiqwotecDataDB + '\n' + error);
            return error;
        }
    }
}
